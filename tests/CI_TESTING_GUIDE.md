...
For unit-test naming and code organization conventions, also see
...
| `nproc_per_node` | Number of distributed processes used by CI |
| `checkout_fetch_depth` | Checkout depth; `0` means full history |
...
`device_types`, rather than `test_matrix.unit.devices`, currently drives the
common workflow matrix. Keep both values consistent in configs that contain
both fields.

The setup script receives these variables:
...
| `gpus_per_node` | Training runner default, normally `8` |
...
bash tests/functional_tests/shell_test_utils/run_ci_test.sh \
  ...
  "GOLDEN_VALUES_PATH=./tests/functional_tests/test_cases/$MODEL/$TEST_CASE/golden_values_dev_dgx_a100.json" \
  "N_REPEAT=1" \
  "ENABLE_LIGHTWEIGHT_MODE=false" \
  "GPUS_PER_NODE=8"
```

For the Hygon benchmark, set:
...
target, repeat count, and GPU count.
