...
def pretrain(
    ...
    start_time_tensor = torch.tensor(
        [_LEGACY_TRAIN_START_TIME], dtype=torch.double, device=timestamp_device
    )
...
def get_model(
    ...
    if (
        ...
        model = [
            to_empty_if_meta_device(
                model_module, device=platform.device(args.local_rank)
            )
            for model_module in model
        ]
        ...
        for _ in range(num_microbatches):
            ...
            if tp_rank == 0:
                ...
                for key in BATCH_KEYS:
                    batch[key] = (
                        batch[key].to(cur_platform.current_device(), non_blocking=True)
